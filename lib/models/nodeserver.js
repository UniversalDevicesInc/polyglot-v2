						...
						newNodeServer.save((err, node) => {
							...
							if (!node) {
								...
								if (newNodeServer.type === 'local') {
									let server = require(node.homeDir + '/server.json')
									if (server.hasOwnProperty('shortPoll')) { node.shortPoll = server.shortPoll }
									if (server.hasOwnProperty('longPoll')) { node.longPoll = server.longPoll }
