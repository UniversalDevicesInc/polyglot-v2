...
NodeServerSchema.methods = {
  ...
  */
  ...
  /*toJSON() {
    let {name, profileNum, nodes, longPoll, shortPoll, executable, serverType, homeDir, token, timeAdded, timeStarted, currentVersion, isConnected, customParams, customData, type, logDir, notices, logPrefix} = this
    return JSON.parse(JSON.stringify({name, profileNum, nodes, longPoll, shortPoll, executable, serverType, homeDir, token, timeAdded, timeStarted, currentVersion, isConnected, customParams, customData, notices, type, logDir, logPrefix}))
  }, */
  ...
  /*async addnode(data, command) {
    try {
      ...
      for (let node of primaryNodes) {
        await doNodeAdd(node, command, data)
      }
      for (let node of data.nodes) {
        doNodeAdd(node, command, data)
      }
      NodeServerModel.sendUpdate()
      this.config()
