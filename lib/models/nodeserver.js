...
NodeServerSchema.statics = {
  ...
  */
  async sendUpdate()  {
    let ns = await NodeServerModel.find({}, null, {sort: {'profileNum': 1}})
    ns.sort((a, b) => {
...
NodeServerSchema.methods = {
  ...
  */
  toJSON() {
    let {name, profileNum, nodes, longPoll, shortPoll, executable, serverType, homeDir, token, timeAdded, isConnected, customParams, customData, type, logDir} = this
    return JSON.parse(JSON.stringify({name, profileNum, nodes, longPoll, shortPoll, executable, serverType, homeDir, token, timeAdded, isConnected, customParams, customData, type, logDir}))
  },
  ...
  async syncNS() {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let doc = await NodeServerModel.findOne({profileNum: this.profileNum})
        config.nodeServers[this.profileNum] = doc
      } catch (err) {
        logger.error(`${this.logPrefix} - Failed to reload NodeServer ${this.name}: ${err.message}`)
      }
      resolve()
    })
  },
  ...
  createQueues() {
    ...
    config.inQ[this.profileNum] = queue()
  },
  ...
  */
  checkCommand(data){
    //if (this.isConnected) {
    let validCommands = ['status', 'addnode','removenode', 'restcall', 'request', 'config', 'connected', 'command', 'customparams', 'installprofile', 'stop', 'start', 'restart', 'customdata']
      for (var prop in data) {
        ...
        if (data.hasOwnProperty(prop)) {
          config.inQ[this.profileNum].push((this[prop] || this['notfound']).call(this, data[prop], prop))
        }
        else {
  ...
  */
  async status(data, command) {
    ...
    isy.handleRequest(this.profileNum, data, command, false, async (result) => {
      if (! result) { return }
      if (result.statusCode === 404) {
        ...
        let success = false
        let ns = await NodeServerModel.findOne({profileNum: this.profileNum, 'nodes.address': data.address})
        let node = ns.nodes.find(n => n.address === data.address)
        if (ns && node) {
  ...
  */
  ...
  async doNodeAdd(node, command, data) {
    ...
    if (newNode.address === newNode.primary) {
      ...
      const doc = await NodeServerModel.findOne({profileNum: this.profileNum, 'nodes.address': newNode.primary})
      if (doc) {
        primaryFound = true
      }
      for (i in this.nodes) {
    ...
    if (!primaryFound) {
      ...
      const result = await isy.handleRequestP(this.profileNum, newNode, command, true)
          ...
          if (this.profileNum === results.isyresponse.connections.connection.profile) {
            ...
            async.each(this.nodes, (node, asyncCb) => {
              if (!node.isprimary) { return asyncCb() }
                isy.handleRequest(this.profileNum, {api: 'nodes/' + isy.addNodePrefix(this.profileNum, node.address)}, 'restcall', true, async (results) => {
                  if (results.statusCode === 404) {
                    logger.info(`${this.logPrefix} ${node.address} doesn't exist in ISY. Adding...`)
                    await this.doNodeAdd(node, 'addnode', {seq: false})
                      .catch(err => asyncCb(err))
                    asyncCb()
                  } else {
  ...
  */
  async config(data = null, command = null) {
    ...
    const doc = await NodeServerModel.findOne({profileNum: this.profileNum})
    config.nodeServers[this.profileNum] = doc
