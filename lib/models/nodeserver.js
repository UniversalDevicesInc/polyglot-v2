...
NodeServerSchema.statics = {
  ...
  */
  async sendUpdate()  {
    let ns = await NodeServerModel.find({})
...
NodeServerSchema.methods = {
  ...
  async syncNS() {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let doc = await NodeServerModel.findOne({_id: this._id})
  ...
  */
  async addnode(data, command) {
    ...
    try {
      await Promise.all(primaryNodes
        .map(node => this.doNodeAdd(node, command, data)))
      await helpers.wait(200)
      await Promise.all(data.nodes
        .map((node) => this.doNodeAdd(node, command, data)))
    } catch (err) {
  ...
  async doNodeAdd(node, command, data) {
    return new Promise(async (resolve, reject) => {
      ...
      if (newNode.address === newNode.primary) {
        ...
        const doc = await NodeServerModel.findOne({'nodes.address': newNode.primary})
        if (doc) {
          primaryFound = true
        }
        for (i in this.nodes) {
      ...
      if (!primaryFound) {
        ...
        isy.handleRequest(this.profileNum, newNode, command, true, async (result) => {
          ...
          if (newNode.added === true) {
            let reason
            const raw = await NodeServerModel.updateOne({'nodes.address': newNode.address}, { $set: { 'nodes.$': newNode }})
            if (raw.nModified === 0) {
              await NodeServerModel.update({_id: this._id, 'nodes.address': { $ne: newNode.address}}, { $push: { nodes: newNode }})
