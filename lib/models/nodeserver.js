...
NodeServerSchema.statics = {
	...
	*/
	checkExistingNodeServers(callback) {
      ...
      logger.debug(`Connection - ${JSON.stringify(results)}`)
			if (!results) { if (callback) { return callback(null, {}) } else return }
			try {
				let i = 0
	...
	*/
	verifyNonManagedNodeServers() {
		NodeServerModel.checkExistingNodeServers((err, isyNodeServers) => {
			logger.debug(`vNMN: ${JSON.stringify(isyNodeServers)}`)
			if (err) { return logger.error(`vNMN: ${err} ${isyNodeServers}`) }
