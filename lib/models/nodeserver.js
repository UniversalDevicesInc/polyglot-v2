...
NodeServerSchema.statics = {
  ...
  */
  async loadNodeServers() {
    try {
      let nodes = await NodeServerModel.find({})
      if (nodes) {
        let i
      ...
      await NodeServerModel.verifyNonManagedNodeServers()
...
NodeServerSchema.methods = {
  ...
  */
  async status(data, command) {
    ...
    let result
    try {
      result = await isy.handleRequestP(this.profileNum, data, command, false)
    } catch (err) {
  ...
  */
  async command(data, command) {
    let result
    try {
      result = await isy.handleRequestP(this.profileNum, data, command, true)
    } catch (err) {
