  ...
  */
  parseFrontend(message) {
    if (message.hasOwnProperty('addns')) {
      ...
      config.nodeServers[message.removenode.profileNum].removenode(message.removenode, 'removenode', (err, result) => {
        if (err) { return mqtt.nsResponse(message, false, err) }
        else { return mqtt.nsResponse(message, true, result) }
      })
    }
...
NodeServerSchema.methods = {
  ...
  */
  removenode(data, command, callback = null) {
    isy.handleRequest(this.profileNum, data, command, true, (result) => {
      if (! result ) { if (callback) { return callback(`ISY did not respond.`) } else { return } }
      let success = false
      let reason = null
      let message = null
      if (result.statusCode === 200) {
        ...
        if (node) {
          node.remove((err) => {
            if (err) {
              ...
              if (callback) { return callback(null, message) } else { return }
            }
          })
        } else {
        ...
        if (result.isyresponse.RestResponse.reason.code === "5106") {
          ...
          if (callback) { return callback(reason) } else { return }
        ...
        if (callback) { return callback(`ISY sent invalid result code.`) } else { return }
      }
    })
  },
  ...
  async getNodesFromISY() {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        logger.error(`getNodesFromISY Error: ${e}`)
        resolve()
      }
    })
  },
  ...
  */
  async connected(data, command) {
    return new Promise(async (resolve, reject) => {
      const doc = await NodeServerModel.findOne({profileNum: this.profileNum}).exec()
      doc.isConnected = data
      await doc.save()
      let primary = doc.nodes.find(n => n.isprimary === true)
      if (primary && primary.added && !primary.isConnected) {
        let update = {
