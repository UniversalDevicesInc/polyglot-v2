...
NodeServerSchema.statics = {
	...
	*/
	sendUpdate()  {
		...
		mqtt.publish('udi/polyglot/frontend/nodeservers', {node: 'polyglot', nodeservers: ns}, {retain: false})
	},
	...
	*/
	addns(name, profileNum, callback, type = 'local') {
		let nsData = {
			name: name,
		...
		if (type === 'local') {
			let nsDir = `${os.homedir()}/.polyglot/nodeservers/${name}/`
			...
			if (fs.existsSync(nsDir + '/server.json') && fs.existsSync(nsDir + '/profile.zip')) {
				...
				if (callback) { return callback(null, {success: false, msg: `${name} local NodeServer server.json or profile.zip not found.`}) } else { return }
			}
		}
					...
					if (err) {
						...
						newNodeServer.save((err, node) => {
							...
							if (!node) {
								...
								logger.info('Registered new ' + type + ' NodeServer: ' + node.name + '(' + node.profileNum + ')')
	...
	parseFrontend(message) {
		if (message.hasOwnProperty('addns')) {
			...
			if (fs.existsSync(nsDir)) {
				var nsDirs = helpers.dirs(nsDir)
				var nsTypes = {}
				nsDirs.forEach((nodeType) => {
					if (fs.existsSync(nsDir + nodeType + '/server.json')) {
						if (!(fs.existsSync(nsDir + nodeType + '/.installed'))) {
							nsTypes[nodeType] = require(nsDir + nodeType + '/server')
						}
					}
				})
				let response = {
