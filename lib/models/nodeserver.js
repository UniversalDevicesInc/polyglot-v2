	...
	parseFrontend(message) {
		if (message.hasOwnProperty('addns')) {
			...
			if (message.customparams.hasOwnProperty('profileNum')) {
				...
				let nsTypes = NodeServerModel.getInstalledNodeTypes()
				let response = {
					node: 'polyglot',
					nodetypes: nsTypes
				}
			...
			NodeServerModel.cloneNodeServer(message, (err, result) => {
				if (err) {
					logger.error(`NS: Error - Failed to Clone NS: ${err}`)
					return mqtt.publish('udi/polyglot/frontend/nodeservers', result)
				} else {
					logger.info(`NS: ${result.msg}`)
					mqtt.publish('udi/polyglot/frontend/nodeservers', result)
				}
			})
		}
	},

	cloneNodeServer(message, callback) {
		if (message.installns.hasOwnProperty('name') && message.installns.hasOwnProperty('url')) {
			let response = {
				...
				response: {
					success: true,
					msg: 'Clone successful. Add the nodeserver to ISY in the "Add NodeServer" tab.'
				}
			}
			let nsPath = `${os.homedir()}/.polyglot/nodeservers/${message.installns.name}/`
			logger.debug(`${message.installns.url} - ${message.installns.name}`)
			let git = require('nodegit').Clone(message.installns.url, nsPath)
			git.then((repo) => {
			...
			callback('name or url not found', response)
		}
	},
...
NodeServerSchema.methods = {
	...
	*/
	checkCommand(data){
		//if (this.isConnected) {
		let validCommands = ['status', 'addnode','removenode', 'restcall', 'request', 'config', 'connected', 'command', 'customparams', 'installprofile']
