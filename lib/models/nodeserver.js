...
NodeServerSchema.statics = {
  ...
  */
  getNodeByName(name, callback) {
    let query = {name: name}
    return NodeServerModel.findOne(query, callback).exec()
  },
  ...
  */
  async loadNodeServers(callback) {
    return new Promise(async (resolve, reject) => {
      try {
        const nodes = await NodeServerModel.find({}).exec()
        if (nodes) {
          let i
...
NodeServerSchema.methods = {
  ...
  */
  async status(data, command) {
    return new Promise(async (resolve, reject) => {
      ...
      const result = await isy.handleRequestP(this.profileNum, data, command, false)
      if (result && result.statusCode === 404) {
        ...
        let success = false
        const ns = await NodeServerModel.findOne({profileNum: this.profileNum, 'nodes.address': data.address}).exec()
        const node = ns.nodes.findIndex(n => n.address === data.address)
        if (node > -1) {
  ...
  */
  async command(data, command) {
    const result = await isy.handleRequestP(this.profileNum, data, command, true)
    if (! result ) { return }
  ...
  */
  async getNodesFromISY(callback) {
    const results = await isy.handleRequestP(this.profileNum, {api: 'profiles/ns/' + this.profileNum + '/connection'}, 'restcall', true)
            ...
            if (!node.isprimary) { return asyncCb() }
              isy.handleRequest(this.profileNum, {api: 'nodes/' + isy.addNodePrefix(this.profileNum, node.address)}, 'restcall', true, async (results) => {
                if (results.statusCode === 404) {
                  ...
                  this.doNodeAdd(node, 'addnode', {seq: false})
                  asyncCb()
                } else {
