		...
		if (message.hasOwnProperty('addns')) {
			...
			if (message.customparams.hasOwnProperty('profileNum')) {
				if (config.nodeServers[message.customparams.profileNum]) {
					config.nodeServers[message.customparams.profileNum].customparams(message.customparams, (err, result) => {
						if (message.hasOwnProperty('seq')) {
							let response = {
								node: 'polyglot',
								seq: message.seq,
...
NodeServerSchema.methods = {
	...
	*/
	toJSON() {
		let {name, profileNum, nodes, token, encryptPayload, time_added, isConnected, customParams} = this
		return JSON.parse(JSON.stringify({name, profileNum, nodes, token, encryptPayload, time_added, isConnected, customParams}))
	},
	...
	*/
	checkCommand(data){
		//if (this.isConnected) {
		let validCommands = ['status', 'addnode','removenode', 'restcall', 'request', 'config', 'connected', 'command', 'customparams']
  ...
  {"result":{"isyresponse":"","statusCode":404,"seq":false,"elapsed":"9.735427ms","profileNum":"8","status":{"success":false,"reason":"lifx_1234 or GV18 does not exist","address":"lifx_1234"}}}
	...
	* @returns {object} response -
	...
	Fail: {"result":{"isyresponse":"","statusCode":200,"seq":false,"elapsed":"25.251796ms","profileNum":"8","addnode":{"success":false,"reason":"lifx_1235 already exists on ISY","address":"lifx_1235"}}}
	*/
	customparams(data, callback) {
		logger.info('Recieved updated Custom Parameters from Frontend Webserver. Updating database.')
		delete data.profileNum
		logger.debug(data)
		this.customParams = data
		this.save()
		callback(null, 'Custom Parameters saved to database sucessfully.')
	},
