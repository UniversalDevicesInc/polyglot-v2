...
NodeServerSchema.statics = {
	...
	*/
	verifyNonManagedNodeServers() {
		if (!config.settings.isyConnected) { return callback() }
...
NodeServerSchema.methods = {
	...
	*/
	toJSON() {
		let {name, profileNum, nodes, token, encryptPayload, timeAdded, isConnected, customParams, type, logDir} = this
		return JSON.parse(JSON.stringify({name, profileNum, nodes, token, encryptPayload, timeAdded, isConnected, customParams, type, logDir}))
	},
				...
				function (result, callback) {
					isy.handleRequest(this.profileNum, {api: 'nodes/' + isy.addNodePrefix(this.profileNum, node.address), seq: data.seq ? data.seq : false}, 'restcall', false, (results) => {
						if (results.statusCode !== 404) {
							...
							let foundnode = this.nodes.find(n => n.address === node.address)
							if (foundnode) {
								foundnode.remove(this.save((err) => {
									if (err) return callback(err)
									return callback()
								}))
							}
