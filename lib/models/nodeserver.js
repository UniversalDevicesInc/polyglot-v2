	...
	doNodeAdd(node, command, data, finishcb){
			...
			async.waterfall([
				...
				function (result, callback) {
					isy.handleRequest(this.profileNum, {api: 'nodes/' + isy.addNodePrefix(this.profileNum, node.address), seq: data.seq ? data.seq : false}, 'restcall', false, (results) => {
						if (results.statusCode !== 404) {
							logger.info(`${node.address} already exists on ISY overwriting Polyglot database with new values from this add`)
							if (result) {
              ...
              if (result.isyresponse.RestResponse.reason.code === '5004') {
								let reason = `${newNode.address} already exists in ISY adding to database.`
								newNode.added = true
								this.nodes.push(newNode)
								this.save((err) => {
									if (err) return callback(err)
									let reason = `node ${newNode.address} already in ISY, added to DB successfully.`
