	...
	*/
	parseFrontend(message) {
		if (message.hasOwnProperty('addns')) {
			...
			NodeServerModel.cloneNodeServer(message, (err, git) => {
				if (err) {
					logger.error(`NS: Error - Failed to Clone NS: ${err.response.msg}`)
	...
	deleteNSFolder(message, callback) {
		...
		if (config.nodeServers.filter((ns) => { return ns.name === message.uninstallns.name })) {
