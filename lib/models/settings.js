...
SettingsSchema.statics = {
  sendUpdate ()  {
    cleanSettings = this.cleanSettings()
  ...
  async updateSettings (newSettings, callback) {
    if (newSettings.hasOwnProperty('updateprofile')) {
      ...
      const options = { new: true, upsert: true }
      this.findOneAndUpdate(query, settings, options, async (err, doc) => {
  ...
  async resetToDefault () {
    ...
    if (settings.hasOwnProperty('ISY_HOST')) {
      ...
      await this.findOneAndUpdate(query, upsertData, options)
  ...
  async loadSettings() {
    let dbsettings = await this.findOne({name: 'polyglot'})
    if (dbsettings) {
