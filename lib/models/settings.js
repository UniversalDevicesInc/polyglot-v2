...
const SettingsSchema = mongoose.Schema({
  ...
  'ipAddress': {
    type: String,
    default: process.env.HOST_IP || (ip.address() || '127.0.0.1')
  },
  'bindIPAddress': {
    type: String,
  ...
  'listenPort': {
    type: Number,
    default: process.env.HOST_PORT || process.env.POLISY ? 443 : 3000
  },
  'useHttps': {
    type: Boolean,
  ...
  'isPolisy': {
    type: Boolean,
    default: process.env.POLISY || false
  }
}, { usePushEach: true })
...
SettingsSchema.statics = {
  ...
  async resetToDefault () {
    try {
      ...
      if (settings.hasOwnProperty('ISY_HOST')) {
        ...
        upsertData = newSettings.toObject()
        ...
        let [isyFound, isyAddress, isyPort] = await findisy.find()
      ...
      logger.error(`resetToDefault: ${err}`)
    }
  },
  ...
  async parseEnvUpdate() {
    try {
      let settings = config.dotenv.parsed || {}
      config.settings.customSSL = false
      if (settings.hasOwnProperty('HOST_IP')) { config.settings.ipAddress = settings.HOST_IP } else { config.settings.ipAddress = (ip.address() || '127.0.0.1') }
      ...
      if (settings.hasOwnProperty('USE_BETA')) { config.settings.useBeta = settings.USE_BETA ? true : false}
      config.settings.useAlpha = settings.USE_ALPHA ? true : false
      if (settings.hasOwnProperty('POLISY') || process.env.POLISY) { config.settings.isPolisy = true }
      else if (!settings.hasOwnProperty('POLISY') && !process.env.POLISY) { config.settings.isPolisy = false }
      if (config.settings.isPolisy) logger.info('Running in Polisy mode')
