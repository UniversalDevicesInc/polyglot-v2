...
module.exports = {
  ...
  toJSON(profileNum) {
    let {name, longPoll, shortPoll, executable, serverType, homeDir, token, timeAdded, timeStarted, currentVersion, isConnected, customParams, customData, type, logDir, notices} = config.nodeServers[profileNum]
    return JSON.parse(JSON.stringify({name, profileNum, longPoll, shortPoll, executable, serverType, homeDir, token, timeAdded, timeStarted, currentVersion, isConnected, customParams, customData, notices, type, logDir}))
  },
  ...
  async checkCommand(data) {
    let validCommands = ['status', 'addnode','removenode', 'restcall', 'request', 'config', 'connected', 'command', 'customparams', 'installprofile', 'stop', 'start', 'restart', 'customdata', 'addnotice', 'removenotice', 'change']
  ...
  async sendControllerState(profileNum, state) {
    if (!config.shutdown) {
      let controller = await Node.findOne({profileNum: profileNum, controller: true}, null, { lean: true })
      if (!controller) {
  ...
  async addnode(profileNum, data, command) {
    ...
    try {
      ...
      logger.error(`${ns.logPrefix} Error adding node: ${err.message}`)
    }
  },
  ...
  async addnotice(profileNum, data, command) {
    try {
      let ns = config.nodeServers[profileNum]
      await NodeServer.findOneAndUpdate({profileNum: ns.profileNum}, {$push: {notices: data}}).exec()
      logger.info(`NS addnotice: ${ns.name} added notice sucessfully: ${data}`)
      this.sendUpdate()
      this.config(profileNum)
    } catch (err) {
  ...
  async removenotice(profileNum, data, command) {
    try {
      let ns = config.nodeServers[profileNum]
      await NodeServer.findOneAndUpdate({profileNum: ns.profileNum}, {$pull: {notices: data}}).exec()
  ...
  async customparams(profileNum, data, command) {
    ...
    try {
      logger.info(`NS: Recieved updated Custom Parameters for ${ns.name}. Updating database.`)
  ...
  async customdata(profileNum, data, command) {
    logger.info(`NS: Recieved updated Custom Data for ${config.nodeServers[profileNum].name}. Updating database.`)
  ...
  async polls(data, command) {
    ...
    child.stopPolls(profileNum)
    child.startPolls(ns)
    logger.info(`NS: Recieved updated Poll Parameters for ${ns.name}. Short: ${data.shortPoll} Long: ${data.longPoll}`)
    result.success = true
