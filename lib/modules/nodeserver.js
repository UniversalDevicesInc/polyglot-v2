...
module.exports = {
    ...
    try {
      ...
      if (result && result.statusCode == 400) {
        ...
        if (result.isyresponse.RestResponse.reason.code === '5004') {
          ...
          this.sendResult(command, {profileNum: profileNum}, false, reason, {address: newNode.address})
  ...
  async removenode(data, command, frontend) {
    ...
    try {
      ...
      logger.debug(JSON.stringify(data), command, frontend)
  ...
  async request(profileNum, data, command) {
    let ns = config.nodeServers[profileNum]
    const result = await isy.handleRequestP(ns.profileNum, data, command)
    if (! result ) { return }
    if (result.statusCode === 404 || result.statusCode === 400) {
  ...
  async customparams(data, command) {
    ...
    try {
      ...
      await NodeServer.findOneAndUpdate({profileNum: ns.profileNum}, {customParams: data}).exec()
      this.config(profileNum)
      result.success = true
