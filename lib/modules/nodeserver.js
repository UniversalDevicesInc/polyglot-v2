...
module.exports = {
  ...
  toJSON(profileNum) {
    let {name, longPoll, shortPoll, executable, serverType, homeDir, token, timeAdded, timeStarted, currentVersion, isConnected, customParams, customData, type, logDir, notices, features, customParamsDoc } = config.nodeServers[profileNum]
    return JSON.parse(JSON.stringify({name, profileNum, longPoll, shortPoll, executable, serverType, homeDir, token, timeAdded, timeStarted, currentVersion, isConnected, customParams, customData, notices, type, logDir, features, customParamsDoc }))
  },
  ...
  async checkCommand(data) {
    let validCommands = ['status', 'addnode','removenode', 'restcall', 'request', 'config', 'connected', 'command', 'customparams', 'customparamsdoc', 'features', 'installprofile', 'stop', 'start', 'restart', 'customdata', 'addnotice', 'removenotice', 'change']
  ...
  async addnode(profileNum, data, command) {
    ...
    try {
      if (!Array.isArray(data.nodes)) return logger.error(`${ns.logPrefix} nodes must be an array.`)
      for (let node of data.nodes) {
        if (await this.updateNode(node, command, data, profileNum)) return
        else {
          if (node.address !== node.primary) await helpers.wait(500)
  ...
  async newNodeAdd(node, command, data, profileNum) {
    ...
    try {
      ...
      if (!config.nodes[profileNum]) config.nodes[profileNum] = {}
  ...
  async config(profileNum, data = null, command = null) {
    ...
    if (ns) {
      ...
      if (!this.supportsFeature(ns, 'noticeByKey')) {
        parseConfig.notices = Object.keys(parseConfig.notices).map(key => ns.notices[key]);
      }
  ...
  async addnotice(profileNum, data, command) {
    try {
      let ns = await NodeServer.findOne({ profileNum: profileNum }).exec()
      this.convertNotices(ns);

      if (typeof data === 'string') {
        ns.notices[this.getMaxNoticeKey(ns)] = data;
      } else if (data.key == null) {
        ns.notices[this.getMaxNoticeKey(ns)] = data.value;
      } else {
        ns.notices[data.key] = data.value;
      }
      ns.markModified('notices');
      await ns.save();
  ...
  async customparams(profileNum, data, command) {
    //let profileNum = data.profileNum
    let ns = config.nodeServers[profileNum]
    let result = {
      message: null,
      success: false,
    ...
    try {
      logger.info(`NS: Received updated Custom Parameters for ${ns.name}. Updating database.`)
      delete data.profileNum
      await NodeServer.findOneAndUpdate({profileNum: profileNum}, {customParams: data}).exec()
      this.config(profileNum)
      this.sendUpdate()
      result.success = true
  ...
  async customdata(profileNum, data, command) {
    logger.info(`NS: Received updated Custom Data for ${config.nodeServers[profileNum].name}. Updating database.`)
    await NodeServer.findOneAndUpdate({profileNum: profileNum}, {customData: data}).exec()
    this.config(profileNum)
  },
  ...
  async features(profileNum, data, command) {
    let features = {};
    for (let feature of Object.keys(data)) {
      let configFeature = config.settings.features[feature];
      if (configFeature != null && configFeature !== 'off') {
        if (data[feature] === 'on') {
          features[feature] = configFeature;
        } else if (data[feature] == 'deprecated') {
          features[feature] = 'deprecated';
        }
      }
    }
    ...
    try {
      let ns = config.nodeServers[profileNum]
      await NodeServer.findOneAndUpdate({profileNum: ns.profileNum}, {features: features}).exec()
      logger.info(`NS features: ${ns.name} saved sucessfully: ${JSON.stringify(features)}`)
      this.sendUpdate()
      this.config(profileNum)
    } catch (err) {
      logger.error(`NS features error: ${err}`)
    }
  },

  supportsFeature(ns, feature) {
    let featureSupport = ns.features[feature];
    if (featureSupport == null || featureSupport === 'off') {
      return false;
    }

    if (featureSupport === 'deprecated') {
      logger.warn('Deprected feature ' + feature + '. Update polyglot');
    }
  ...
  async customparamsdoc(profileNum, data, command) {
    try {
      let ns = config.nodeServers[profileNum]
      await NodeServer.findOneAndUpdate({profileNum: ns.profileNum}, {customParamsDoc: data}).exec()
      logger.info(`NS customparamsdoc: ${ns.name} saved sucessfully`)
      this.sendUpdate()
      this.config(profileNum)
    } catch (err) {
      logger.error(`NS error: ${err}`)
    }
  },
