...
module.exports = {
        ...
        for (let node of nodes) {
          try {
            ...
            if (result.statusCode === 404) {
              ...
              await this.newNodeAdd(node, 'addnode', {seq: false}, ns.profileNum)
            } else {
    ...
    try {
      ...
      for (let node of data.nodes) {
        if (node.hint === undefined) {
          node.hint = '0x00000000';
        } else {
          node.hint = isy.getHint(node.hint);
        }
  ...
  async newNodeAdd(node, command, data, profileNum) {
    ...
    let newNode = {
      ...
      name: node.name,
      profileNum: profileNum,
      nodedef: node.node_def_id || 'controller',
 ...
 /* ======================================================================= */
  ...
  async restoreNodeServer(ns) {
    logger.info(`RESTORE: Overwritting slot ${ns.profileNum} on ISY with ${ns.name}`)
    let args = {
    ...
    let res = await isy.handleRequestP(ns.profileNum, {api: 'profiles/ns/' + ns.profileNum + '/connection/set/network', args: args}, 'restcall', true)
    if (res.statusCode !== 200) throw new Error (`Failed to install ${ns.name} into slot ${ns.profileNum}. ISY responded with code ${res.statusCode}. Check your ISY settings then try the restore again.`)
    logger.info(`RESTORE: ${ns.name} configured in slot ${ns.profileNum} on ISY Sucessfully`)
  },
  ...
  async delns(data, command) {
    ...
    try {
      ...
      if (results.statusCode === 200) {
        Node.deleteMany({ profileNum: data.profileNum }).exec()
        NodeServer.deleteOne({ profileNum: ns.profileNum }).exec()
          ...
          for (let nodeServer of Object.values(config.nodeServers)) {
            if (nodeServer) {
              if (!(foundNS.hasOwnProperty(nodeServer.profileNum)))  {
                logger.info(`NS: ${nodeServer.name} NodeServer no longer found in the ISY. Removing from Polyglot`)
                if (nodeServer.type !== 'unmanaged') {
                  Node.deleteMany({ profileNum: data.profileNum }).exec()
                }
                NodeServer.deleteOne({ profileNum: nodeServer.profileNum }).exec()
