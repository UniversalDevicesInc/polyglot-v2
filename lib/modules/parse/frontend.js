  ...
  for (let key in message) {
    ...
    try {
      ...
      if (command.type === 'nsmethod') {
        console.log(config.nodeServers[message[key].profileNum])
        result = await config.nodeServers[message[key].profileNum][command.func](message[key], key, true).bind(NodeServer)
      } else {
        result = await command.func.call(NodeServer, message[key])
      }
