...
var _fileName = {
  ...
  freebsd: 'polyglot-v2-freebsd-x64.tar.gz'
}
...
async function upgradeInterfaces() {
  let runCmd = `curl -s https://raw.githubusercontent.com/UniversalDevicesInc/polyglot-v2/master/scripts/upgrade_interfaces.sh | bash -e`
  let opts = {
    cwd: os.homedir(),
    shell: '/bin/sh'
  }
...
async function polisyUpgrade() {
  let polyglotPackage = 'polyglot'
  if (config.settings.useAlpha) polyglotPackage = 'polyglot-alpha'
  else if (config.settings.useBeta) polyglotPackage = 'polyglot-beta'
  let runCmd = `sudo pkg update && sudo ASSUME_ALWAYS_YES=YES pkg remove polyglo* && sudo ASSUME_ALWAYS_YES=yes pkg install ${polyglotPackage}`
  let opts = {
    cwd: os.homedir(),
    shell: '/bin/sh'
  }
  mqttc.publish(_upgradeTopic, {node: 'polyglot', progress: { percent: 0.25 }})
  logger.debug(`polisyUpgrade: Starting pkg upgrade`)
  try {
    await spawn('PKG', runCmd, [], opts)
  } catch (err) {
    logger.error(`polisyUpgrade: ${err.stack}`)
  }
  _inProgress = false
  mqttc.publish(_upgradeTopic, {node: 'polyglot', progress: { percent: 0.75 }})
...
function getSystem() {
 ...
 let arch = null
 if (['darwin', 'freebsd'].includes(platform)) {
   if (platform === 'freebsd' && config.settings.isPolisy) {
    return polisyUpgrade()
   } else return download(platform)
