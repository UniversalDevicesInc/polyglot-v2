 ...
 */
  ...
  async handleRequestP(profileNum, data, command, gettext = false) {
    ...
    if (url) {
      try {
        return await this.getIsy(profileNum, url, data.seq ? data.seq : false, gettext)
      } catch (err) {
  ...
  addnode(profileNum, data, cmd){
    ...
    if (data.address && data.primary && data.name && data.nodedef) {
      ...
      url = this.makeNodeUrl(profileNum, path, queryargs)
    } else { return logger.error(`ISY: Error address, primary, nodedef, or name not found in ${cmd} call. ${JSON.stringify(data)}`) }
    return url
  },
  ...
  setHint(profileNum, data, cmd) {
    let url = false
    if (data.address) {
      var path = [ 'nodes', data.address, 'set', 'hint', this.getHint(data.hint) ];
      url = this.makeNodeUrl(profileNum, path)
    } else { logger.error(`ISY: address not found in ${cmd} call.`) }
    return url
  },
  /**
   * Converts hint from array to string (if necessary)
   * @method
   * @param hint
   */
  getHint(hint) {
