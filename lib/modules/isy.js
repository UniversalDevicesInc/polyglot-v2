 ...
 */
  ...
  async handleRequestQ(profileNum, data, command, gettext = false, callback) {
    var url = (this[command] || this['notfound']).call(this, profileNum, data, command)
    if (url) {
      try {
        config.isyQ.push(async (callback) => {
          let result = await this.getIsy(profileNum, url, data.seq ? data.seq : false, gettext)
          callback(result)
        })
      } catch (err) {
        logger.error(`ISY: ${err}`)
