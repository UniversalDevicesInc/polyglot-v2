        ...
        try {
          ...
          if (server.hasOwnProperty('install')) {
            ...
            installProcess.stdout.on('data', (data) => {
              logger.debug(`NSChild: ${ns.name} Install : ${data}`)
            })
            installProcess.on('close', (code) => {
              logger.debug(`NSChild: ${ns.name} install process exited with cause code: ${code}`)
            })
            installProcess.on('error', (err) => {
              logger.debug(`NSChild: ${ns.name} install process error: ${err}`)
            })
          } else {
            logger.debug(`NSChild: ${ns.name} does not have an install process defined. Run any install scripts for the nodeserver manually if necessary.`)
          }
        } catch (e) {
        ...
        try {
          ...
          if (server.hasOwnProperty('type') && server.hasOwnProperty('executable')) {
            ...
            try {
              ...
              this.nodeProcesses[ns.profileNum].on('EPIPE', (err) => {
