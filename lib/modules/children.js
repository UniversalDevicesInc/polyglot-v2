        ...
        try {
          ...
          if (server.hasOwnProperty('type') && server.hasOwnProperty('executable')) {
            ...
            try {
              ...
              this.nodeProcesses[ns.profileNum].stderr.on('error', (err) => {
                this.stopPolls(ns)
                logger.error(`NSChild ${config.nodeServers[ns.profileNum].name} STDERR(error): ${err}`)
                delete this.nodeProcesses[ns.profileNum]
              })
              this.nodeProcesses[ns.profileNum].stdout.on('error', (err) => {
                this.stopPolls(ns)
                logger.error(`NSChild ${config.nodeServers[ns.profileNum].name} STDOUT(error): ${err}`)
                delete this.nodeProcesses[ns.profileNum]
              })
              ...
              setTimeout(() => {
                if (this.nodeProcesses[ns.profileNum]) {
                  try {
                    ...
                    logger.error(`NSChild: Error writing config to child. This typically means it crashed before we could.`)
                  }
                }
              }, 500)
            } catch (e) {
