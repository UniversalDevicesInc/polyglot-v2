...
module.exports = {
        ...
        if (message.updatens.hasOwnProperty('updateProfile')) {
          ...
          config.nodeServers.forEach(ns => {
            if (ns.name === message.updatens.name && ns.type === 'local') {
              found = true
              ns.installprofile(null, null, (err, result) => {
  ...
  startPolls(ns) {
    ns.shortPollTimer = setInterval(() => { ns.sendPoll('shortPoll') }, ns.shortPoll * 1000)
    ns.shortPollTimer.unref()
    ns.longPollTimer = setInterval(() => { ns.sendPoll('longPoll') }, ns.longPoll * 1000)
    ns.longPollTimer.unref()
  },

  stopPolls(ns) {
    clearInterval(ns.shortPollTimer)
    clearInterval(ns.longPollTimer)
  },
            ...
            try {
              ...
              this.nodeProcesses[ns.profileNum].on('close', (code) => {
                if (config.nodeServers[ns.profileNum]) {
                  ...
                  logger.debug(`NSChild: NodeServer Shutdown cause code: ${code}`)
                }
                delete this.nodeProcesses[ns.profileNum]
              })
