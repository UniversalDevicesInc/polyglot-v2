...
router.get('/profile', passport.authenticate('jwt', {session: false}), (req, res, next) => {
  ...
  */
  ...
  router.get('/log/:id', passport.authenticate('jwt', {session: false}), async (req, res, next) => {
    ...
    let response
    let ns
    if (req.params.id.toLowerCase() === 'polyglot') logFile = os.homedir() + '/.polyglot/log/debug.log'
    else {
  ...
  router.get('/log/package/:id', passport.authenticate('jwt', {session: false}), async (req, res, next) => {
    ...
    if (ns && ns.type === 'local') {
      ...
      let polylogFile = os.homedir() + '/.polyglot/log/debug.log'
      let polylogFile2 = os.homedir() + '/.polyglot/log/debug.log.0'
