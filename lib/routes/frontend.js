  ...
  */
  ...
  router.get('/backup', passport.authenticate('jwt', {session: false}), async (req, res, next) => {
    try {
      ...
      console.log(master.nodeServers)
      ...
      res.set({
        'access-control-expose-headers': 'content-disposition',
        'content-type': 'unkown',
        'content-disposition': `attachment;filename=polyglot-backup-${tsFormat()}.bin`,
  ...
  router.post('/restore', passport.authenticate('jwt', {session: false}), async (req, res, next) => {
    try {
      logger.info(`RESTORE: Procesisng restore request. File Name: ${req.files.file.name}`)
      let decrypted = JSON.parse(encrypt.decryptText(req.files.file.data.toString()))
      ...
      const storeNodeServers = JSON.stringify(storeRes.body)
      if (decrypted.nodeServers && decrypted.nodeServers.length > 0) {
        for (let ns of decrypted.nodeServers) {
          ...
          if (config.nodeServers[ns.profileNum]) {
            logger.info(`RESTORE: NodeServer already provisioned in slot ${ns.profileNum}: ${config.nodeServers[ns.profileNum].name}`)
          ...
          let result = await NodeServer.findOneAndUpdate({name: ns.name}, ns, { upsert: true })
          if (result.ok === 1) {
            logger.info(`RESTORE: NodeServer (${ns.profileNum}) - ${ns.name} complete. Processing nodes...`)
            let deleted = await Node.deleteMany({ profileNum: 4})
            ...
            for (let node of decrypted.nodes[ns.profileNum]) {
              let result = await Node.findOneAndUpdate({ address: node.address }, node, { upsert: true })
